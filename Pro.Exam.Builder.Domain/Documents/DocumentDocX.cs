...
namespace Pro.Exam.Builder.Domain
{
    public class DocumentDocX : IDocumentDocX
    {
        ...
        public ExamLinks CreateDocument(QuestionsDto question)
        {
            ...
            if (File.Exists(Path.Combine(path, headerImage)))
            {
                using (FileStream picData = new FileStream(Path.Combine(path, headerImage), FileMode.Open, FileAccess.Read))
                {
                    var image = doc.CreateParagraph();
                    XWPFRun imageRun = image.CreateRun();
                    imageRun.AddPicture(picData, (int)PictureType.PNG, "header.PNG", (int)(689.0 * 9525), (int)(655.0 * 9525));
                }
            }
